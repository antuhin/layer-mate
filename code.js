...
async function trackAction(type, metadata) {
  if (!metadata) metadata = {};
  try {
    const analytics = await figma.clientStorage.getAsync('analytics') || { actions: [], healthScores: [], issues: {} };
    const actionData = Object.assign({ type: type, timestamp: Date.now() }, metadata);
    analytics.actions.push(actionData);
    const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
    analytics.actions = analytics.actions.filter(function (a) { return a.timestamp > thirtyDaysAgo; });
    await figma.clientStorage.setAsync('analytics', analytics);
  } catch (error) {
    console.error('Analytics error:', error);
  }
}

async function trackHealthScore(score, totalChecks, passedChecks, errors) {
  try {
    const analytics = await figma.clientStorage.getAsync('analytics') || { actions: [], healthScores: [], issues: {} };
    analytics.healthScores.push({
      timestamp: Date.now(),
      score: score,
    ...
    if (!analytics.issues) analytics.issues = {};
    analytics.issues.poorContrast = (analytics.issues.poorContrast || 0) + ((errors.poorContrast && errors.poorContrast.length) || 0);
    analytics.issues.smallFonts = (analytics.issues.smallFonts || 0) + ((errors.smallFonts && errors.smallFonts.length) || 0);
    analytics.issues.smallTouchTargets = (analytics.issues.smallTouchTargets || 0) + ((errors.smallTouchTargets && errors.smallTouchTargets.length) || 0);
    analytics.issues.missingAltText = (analytics.issues.missingAltText || 0) + ((errors.missingAltText && errors.missingAltText.length) || 0);
    analytics.issues.defaultNames = (analytics.issues.defaultNames || 0) + ((errors.defaultNames && errors.defaultNames.length) || 0);

    const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
    analytics.healthScores = analytics.healthScores.filter(function (h) { return h.timestamp > thirtyDaysAgo; });
    await figma.clientStorage.setAsync('analytics', analytics);
  } catch (error) {
    console.error('Health score tracking error:', error);
  }
}
...
async function getAnalytics(days) {
  ...
  try {
    ...
    return {
      actions: analytics.actions.filter(function (a) { return a.timestamp > cutoff; }),
      healthScores: analytics.healthScores.filter(function (h) { return h.timestamp > cutoff; }),
      issues: analytics.issues || {}
    };
  } catch (error) {
    return { actions: [], healthScores: [], issues: {} };
  }
}

async function clearAnalytics() {
  await figma.clientStorage.setAsync('analytics', { actions: [], healthScores: [], issues: {} });
  figma.notify('🗑️ Analytics data cleared');
}

function generateAnalyticsCSV(analytics) {
  var csv = 'Type,Data,Timestamp\n';
  analytics.actions.forEach(function (a) {
    csv += 'Action,' + a.type + ',' + new Date(a.timestamp).toISOString() + '\n';
  });
  analytics.healthScores.forEach(function (h) {
    csv += 'Health Score,' + h.score + ',' + new Date(h.timestamp).toISOString() + '\n';
  });
  for (var type in analytics.issues) {
    if (analytics.issues.hasOwnProperty(type)) {
      csv += 'Issue,' + type + ': ' + analytics.issues[type] + ',' + new Date().toISOString() + '\n';
    }
  }
  return csv;
}
  ...
  try {
    switch (msg.type) {
      ...
      case 'find-detached-colors':
        await handleScanDetachedColors();
        break;

      // ========================================
      // DESIGN OS: ARCHITECT MODE MODULES
      // ========================================
      case 'generate-page-shell':
        await handleGeneratePageShell();
        break;

      case 'auto-structure':
        await handleAutoStructure();
        break;

      case 'toggle-visual-overlay':
        await handleToggleVisualOverlay(msg.enabled);
        break;
      ...
      case 'get-analytics':
        const analytics = await getAnalytics(msg.days || 30);
        figma.ui.postMessage({ type: 'analytics-data', analytics });
        break;

      case 'clear-analytics':
        await clearAnalytics();
        figma.ui.postMessage({ type: 'analytics-cleared' });
        break;

      case 'export-analytics-csv':
        const analyticsData = await getAnalytics(30);
        const csv = generateAnalyticsCSV(analyticsData);
        figma.ui.postMessage({ type: 'analytics-csv', csv: csv });
        break;
...
// Note: handleAutoStructure has been replaced by Design OS Module 2 below

// ========================================
// LEGACY: Old Auto-Structure (Replaced by Design OS Module 2)
...
// ========================================
// DESIGN OS: MODULE 2 - SMART ENGINE (Enhanced)
// Auto-structure content with layout detection
// ========================================
// Note: The existing handleAutoStructure is being replaced with enhanced version
